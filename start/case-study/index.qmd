...
Following the original analysis of the ECPE data by @ecpe, we'll estimate a loglinear cognitive diagnostic model (LCDM).
...
In this case, we want to estimate the model using MCMC with the `{rstan}` package as the estimation engine.
...
```{r}
#| label: calc-attribute-base-rate
#| include: false

library(knitr)
library(glue)

att_base_rate <- structural_parameters |>
  summarize(
...
att_summary <- att_base_rate |>
  ...
  glue_data(
    "{fmt_prop_pct(E(value))}% of respondents are proficient on ",
    "{name} rules"
  ) |>
  combine_words()
```
...
For more information on prior distributions, including information on how to specify your own prior distributions for the model parameters, see `?prior` and `vignette("model-estimation", package = "measr")`.

### Respondent Proficiency

```{r}
#| label: add-resp-estimates
#| echo: false
...
att73_text <- att73 |>
  glue_data(
    "a {fmt_prop_pct(probability)}% chance the respondent is ",
...
att73_morpho <- att73 |>
  ...
  glue_data(
    "as low as {fmt_prop_pct(`2.5%`)}% or as high as ",
    "{fmt_prop_pct(`97.5%`)}%"
  )
```
...
That is, we're fairly confident respondent 73 is proficient on lexical rules, somewhat confident they are proficient on cohesive rules, and not confident about whether or not the student is proficient on morphosyntactic rules (i.e., the proficiency probability could reasonably be `r att73_morpho`).
These probabilities can be turned into classifications by setting proficiency thresholds.
For example, we might decide that probabilities greater than .5 (i.e., more likely than not) indicate proficiency [e.g., @bradshaw2019].
